...
Specifies whether to include implicit default constructors in the audit (default: `false`).

The default constructors generated by the compiler are not documented in XML and are typically excluded from audits and documentation.
...
Kampose ensures all hyperlinks referenced in the XML documentation comments (`<see>` and `<seealso>` tags with `href` attributes) and topics are well-formed and valid. For
internal links, it also checks that they point to existing documentation content.
